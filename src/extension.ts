...
const withEditor = (f: (editor: vscode.TextEditor) => void) => () => {
	const editor = vscode.window.activeTextEditor
	if (editor) f(editor)
}

const move = (editor: vscode.TextEditor, num: number) => {
	const newPos = editor.selection.active.translate(0, num)
	editor.selection = new vscode.Selection(newPos, newPos)
}

const getWordAtPosition = (editor: vscode.TextEditor) => {
	const wordRange = editor.document.getWordRangeAtPosition(editor.selection.start)
	if (!wordRange) return
	return editor.document.getText(wordRange);
}

const positionSame = (a: vscode.Position, b: vscode.Position) =>
...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('vanjsHelper.insertSymbol', withEditor(editor => {
		...
		vscode.commands.registerCommand('vanjsHelper.importComponent', withEditor(editor => {
			...
			for (let i = 0; !matches && i < doc.lineCount; ++i) {
				...
				editor.edit(editorBuilder => editorBuilder.replace(
					...
					tmpl.replace("$COMPONENTS", [...new Set([...components, word])].sort().join(", "))
				))
			}

			if (!matches) vscode.window.showErrorMessage("No component import line found.")
		})),
